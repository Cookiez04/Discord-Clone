...
import { X, LogOut, Monitor, Lock, Shield, Key, Smartphone, Moon, Sun, CloudMoon, Palette, ArrowLeft } from 'lucide-react';
...
export const SettingsModal: React.FC<Props> = ({ user, theme, accentColor, onClose, onUpdateTheme, onUpdateAccentColor }) => {
        ...
        return (
            <div className="h-full flex flex-col">
                ...
                <div className="md:hidden mb-4">
                    <button onClick={() => setShowMobileContent(false)} className="flex items-center text-discord-text-muted hover:text-white">
    ...
    return (
        <div className="fixed inset-0 z-50 flex bg-discord-darkest">
            {/* Sidebar */}
            <div className={`
                w-full md:w-[28%] md:min-w-[200px] bg-discord-darker flex-col justify-end pt-14 pr-4 pb-4 pl-4 md:pl-0
                ${showMobileContent ? 'hidden md:flex' : 'flex'}
            `}>
                <div className="w-full md:w-[192px] flex flex-col md:ml-auto">
                    <div className="px-2.5 pb-1.5 text-xs font-bold text-discord-text-muted uppercase">User Settings</div>
                    <SidebarItem label="My Account" isActive={activeTab === 'My Account'} onClick={() => handleTabClick('My Account')} />
                    ...
                    <SidebarItem label="Appearance" isActive={activeTab === 'Appearance'} onClick={() => handleTabClick('Appearance')} />
            ...
            {/* Content */}
            <div className={`
                flex-1 bg-discord-dark pt-14 pl-4 pr-4 md:pl-10 md:pr-10 overflow-y-auto custom-scrollbar relative
                ${showMobileContent ? 'block' : 'hidden md:block'}
            `}>
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 text-discord-text-muted hover:text-white flex flex-col items-center z-10"
                >
                    <div className="border-2 border-discord-text-muted rounded-full p-1 mb-1 hover:border-white">
                        <X size={16} />
                    </div>
                    <span className="text-xs font-bold uppercase hidden md:block">ESC</span>
                </button>
